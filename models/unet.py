...
Triển khai biến thể U-Net cấu hình được (feature_scale, more_layers, concat_x, upsample_mode, pad, norm_layer).
...
class ListModule(nn.Module):
    """Danh sách module indexable (hỗ trợ more_layers)."""
...
class UNet(nn.Module):
    """U-Net cấu hình được dùng làm prior trong DIP.
    ...
    def __init__(
        ...
        # final_activation: None|'sigmoid'|'tanh' overrides need_sigmoid
        super(UNet, self).__init__()
        if need_sigmoid and final_activation is None:
    ...
    def forward(self, inputs):
        ...
        if self.more_layers > 0:
            ...
            for idx in range(self.more_layers - 1):
                up_block = self.more_ups[self.more_layers - idx - 2]
                up_ = up_block(up_, prevs[self.more_layers - idx - 2])
        else:
            up_ = down4
...
class unetConv2(nn.Module):
    """Hai lớp Conv (3x3) + Norm + ReLU nối tiếp cho encoder/decoder."""
...
class unetDown(nn.Module):
    """Block downsampling: MaxPool2d -> unetConv2."""
...
class unetUp(nn.Module):
    """Block upsampling: (ConvTranspose2d hoặc Upsample+Conv) + unetConv2 (với skip)."""
