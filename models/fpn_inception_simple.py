...
class FPNHead(nn.Module):
    ...
    def _init_weights(self):
        nn.init.kaiming_normal_(self.block0.weight, nonlinearity="relu")
        nn.init.kaiming_normal_(self.block1.weight, nonlinearity="relu")
...
class FPNInceptionSimple(nn.Module):
    def __init__(
        self, norm_layer=None, output_ch=3, num_filters=128, num_filters_fpn=256
    ):
        super().__init__()
...
class FPN(nn.Module):
    def __init__(self, norm_layer=None, num_filters=256, freeze_backbone=True):
        """Creates an `FPN` instance for feature extraction.
        Args:
          num_filters: the number of filters in each output pyramid level
          freeze_backbone: whether to freeze the pretrained backbone parameters
        """

        super().__init__()
        norm_layer = norm_layer or nn.BatchNorm2d

        # Defer heavy import and provide a clear error if missing
        try:
            from pretrainedmodels import inceptionresnetv2  # type: ignore

            self.inception = inceptionresnetv2(num_classes=1000, pretrained="imagenet")
        except Exception as e:
            raise ImportError(
                "Backbone 'inceptionresnetv2' not available. Please install 'pretrainedmodels' via 'pip install pretrainedmodels'."
            ) from e
    ...
    def forward(self, x):
        # Bottom-up pathway, from Inception-ResNet-v2
        enc0 = self.enc0(x)
        ...
        # Top-down pathway
        pad = (1, 2, 1, 2)  # reflect pad to align spatial dims before addition
        pad1 = (0, 1, 0, 1)  # reflect pad to match upsampled shapes for output
        map4 = lateral4
        map3 = lateral3 + F.interpolate(map4, scale_factor=2, mode="nearest")
        map2 = F.pad(lateral2, pad, "reflect") + F.interpolate(
            map3, scale_factor=2, mode="nearest"
        )
        map1 = lateral1 + F.interpolate(map2, scale_factor=2, mode="nearest")
